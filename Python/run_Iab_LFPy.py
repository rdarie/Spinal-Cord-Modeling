...
from Iab_network_LFPy import Iab_network as Iab_net
import numpy as np
...
net = Iab_net(POPULATION_SIZE,
                     cellParameters,
                     populationParameters,
                     synapseParameters)
#
ev_list_ia = hf.get_v_from_mat('E:\\Google Drive\\Github\\tempdata\\move_root_um_move_root_points_cs.mat',0)
ev_list_ib = hf.get_v_from_mat('E:\\Google Drive\\Github\\tempdata\\move_root_um_move_root_points_cs.mat',1)
#pdb.set_trace()
#ev_list = np.linspace(0,ev_list[0],20).tolist() + ev_list
#ev_list = ev_list + np.linspace(ev_list[-1],0,20).tolist()
fudge_factor = -2e5
ev = {"Ia" : fudge_factor * np.array(ev_list_ia), "Ib" : fudge_factor * np.array(ev_list_ib)}
...
net.run()
res = net.results
pickle.dump(res, open( "E:\\Google Drive\\Github\\tempdata\\Iab_net.p", "wb" ) )
