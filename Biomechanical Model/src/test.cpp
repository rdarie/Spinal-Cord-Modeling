...
int main(int argc, const char** argv)
{
	...
	std::string = hack("E:\Google Drive\Borton Lab\Biomechanical Model\MuJoCo\Tutorial\src\passive_walker_osim.xml");
	// load model
	mjModel* m = 0;
	...
	if( binary )
		...
		m = mj_loadXML(argv[1], error);
	if( !m )
		return finish(error);
	*/
	m = mj_loadModel(hack, 0, 0);
	// make data
	...
	if( option.find_first_of('x')!=std::string::npos )
	{
		...
		// save
		if( !mj_saveXML(tempfile.c_str(), m, error) )
			return finish(error, m, d);

		// load back
		mjModel* mtemp = mj_loadXML(tempfile.c_str(), error);
		if( !mtemp )
			return finish(error, m, d);
